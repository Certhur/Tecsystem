...
if (empty($_SESSION["username"]) && empty($_SESSION["password"])){
    ...
    if ($_GET["act"]=="insert") {
		if (isset($_POST["Guardar"])) {
            ...
            VALUES('$username','$password','$name_user','$permisos_acceso')")or die('error: '.mysqli_error($mysqli));
            if ($query) {
		...
		if (isset($_POST["Guardar"])) {
			...
			if (isset($_POST["id_user"])) {
				...
				if (empty($_FILES["foto"]["name"])) {
                    ...
                    WHERE id_user = '$id_user'")or die('error: '.mysqli_error($mysqli));
		...
		}echo "<br>No_Guardar ==>".$_POST["Guardar"];
	}	
        elseif ($_GET["act"]=="on") {
		if (isset($_GET["id"])) {
			$id_user = $_GET["id"];
